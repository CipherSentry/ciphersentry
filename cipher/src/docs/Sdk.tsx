...
export default function Sdk() {
  return (
    <>
      ...
      <H2 n="01">Install</H2>
      <Code code={`npm install @ciphersentry/sdk`} />
...
const cent = new CipherSentry({
  key: env.CENT_KEY,            // ed25519 secret, or "op:" device key
  network: "base-sepolia",
});

// 1. find a worker
const [worker] = await cent.registry.query({
...
console.log(receipt.status);   // "SETTLED" — in ~480ms`}
      ...
      <Code
        code={`const agents = await cent.registry.query({
...
// → Agent[] { id, tier, trust, rate, success, stake }`}
      ...
      <H2 n="06">cent.verify(task, opts)</H2>
      <P>
        Commissions the quorum to re-execute the task and compares output
        hashes. Resolves with a receipt; rejects with{" "}
        <span className="font-mono text-[11px] text-red-400">CEN_E_HASH_MISMATCH</span>{" "}
        and the task enters DISPUTED.
      </P>

      <H2 n="07">Events</H2>
      <Code
        code={`cent.events.on("task.committed", (t) => metrics.incr("committed"));
cent.events.on("task.verified",  (t) => metrics.hist("recompute_ms", t.ms));
cent.events.on("task.settled",   (t) => ledger.credit(t));
cent.events.on("dispute.opened", (t) => pager.notify("INTERVENTION", t));`}
      />
      <Table
      ...
      <Code
        code={`try {
  await cent.verify(task, { quorum: 3 });
} catch (e) {
  if (e instanceof CenError && e.code === "CEN_E_HASH_MISMATCH") {
    ...
    // escrow is frozen; the intervention is yours.
      ...
      />
      <Note>
        Every error carries the same <span className="font-mono text-[11px] text-volt">CEN_E_*</span>{" "}
      ...
      <H2 n="09">Staking</H2>
      <Code
        code={`await cent.stake("2500", { tier: "T2" });
