...
const ACCRUAL_START_MS = 1_780_704_000_000; // 2026-06-06T00:00:00Z
const GATE4_DAYS = 60;
const WAITLIST_FLOOR = 349;

const INFRA = ["FIRECRACKER VM", "BARE METAL", "SELF-HOST GPU", "CLOUD K8S"];
const GATEWAY_URL = (
  (import.meta as ImportMeta & { env?: { VITE_GATEWAY_URL?: string } }).env
    ?.VITE_GATEWAY_URL ?? resolveDefaultNode()
).replace(/\/$/, "");
...
/** Unified $CENT page — launch pack + gates + waitlist · #/cent */
export default function Cent() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const h = (await fetch(`${GATEWAY_URL}/health`).then((r) =>
          r.json(),
        )) as { ok?: boolean; access_requests?: number };
        if (cancelled) return;
      ...
      try {
        const s = (await fetch(`${GATEWAY_URL}/access-requests/stats`).then(
          (r) => r.json(),
        )) as { ok?: boolean; count?: number; waitlist?: number };
        if (cancelled) return;
        const n = s.waitlist ?? s.count;
        if (s.ok && typeof n === "number")
          setWaitlistCount(Math.max(WAITLIST_FLOOR, n));
      } catch {
        /* optional */
      }
      try {
        const idx = resolveDefaultIndexer();
        const batches = (await fetch(`${idx}/batches`).then((r) =>
          r.json(),
        )) as { data?: { anchored_block?: number | null }[] };
        if (cancelled) return;
  ...
  const GATES = [
    ...
    },
    {
      ...
      desc: "CENT fair launch on orynth.dev — no presale, no VC token round. Listing legal after audits booked.",
    },
  ];
  ...
  const join = async () => {
    ...
    try {
      ...
      if (typeof data.queue === "number")
        setWaitlistCount((c) => Math.max(c, data.queue!));
      setPhase("done");
    } catch (e) {
  ...
  return (
    <div className="relative isolate min-h-screen bg-void font-display text-mist">
      ...
      {/* G4 accrual strip */}
      <div className="border-b border-edge bg-deepgreen px-4 py-5 sm:px-6 sm:py-6 md:px-12">
        <div className="mx-auto flex max-w-[1240px] flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 font-display text-[26px] font-medium tabular-nums tracking-[-0.03em] text-volt sm:text-[32px]">
              {String(days).padStart(2, "0")}
              <span className="ml-2 font-mono text-[10px] tracking-[0.18em] text-code-mute sm:text-[11px]">
                / {GATE4_DAYS} DAYS
              </span>
            </div>
          </div>
          <div className="w-full max-w-md sm:w-72">
            ...
            <div className="mt-2 flex justify-between gap-2 font-mono text-[7.5px] tracking-[0.12em] text-code-mute/80 sm:tracking-[0.16em]">
              <span className="truncate">GENESIS · BLK 12,840,117</span>
              <span className="shrink-0">ETA ≥ {formatEta(now)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1240px] px-4 py-10 sm:px-6 sm:py-12 md:px-12">
        <div className="mb-6 font-mono text-[9px] tracking-[0.24em] text-volt sm:mb-8">
          READINESS GATES · G1–G5
        </div>
        <div className="grid gap-8 lg:grid-cols-[1fr_min(100%,400px)] lg:gap-10">
          <div className="min-w-0">
            <div className="space-y-3">
              {GATES.map((g) => (
                <div
                  key={g.id}
                  className={`grid gap-3 border p-4 sm:gap-4 sm:p-5 md:grid-cols-[70px_minmax(0,1fr)_auto] md:items-center ${
                    g.id === "G4"
                      ? "border-volt/50 bg-volt/[0.04]"
                      : "border-edge"
                  }`}
                >
                  <span
                    className={`font-mono text-[13px] font-semibold ${
                      g.id === "G4" ? "text-volt" : "text-mute"
                    }`}
                  >
                    {g.id}
                  </span>
                  <div className="min-w-0">
                    <h3 className="font-display text-[15px] font-semibold tracking-[-0.01em] text-mist sm:text-[16px]">
                      {g.name}
                    </h3>
                    <p className="mt-1.5 text-[12px] leading-[1.7] text-mute">
                      {g.desc}
                    </p>
                  </div>
                  <div className="flex flex-row flex-wrap items-center gap-2 md:flex-col md:items-end md:gap-1.5">
                    <Tag
                      tone={
                        g.status === "COUNTING" || g.status === "IN PROGRESS"
                          ? "volt"
                          : g.status === "PENDING"
                            ? "dim"
                            : "amber"
                    ...
                    <span className="font-mono text-[8px] tracking-[0.16em] text-mute/60">
                      {g.metric}
                    </span>
                  </div>
                </div>
            ...
            {/* freeze-hash anchor */}
            <div className="mt-6 border border-amber-600/35 bg-panel p-4 sm:p-5">
              <div className="flex flex-wrap items-center justify-between gap-2 font-mono text-[8.5px] tracking-[0.24em] text-mute">
                <span>SOURCE FREEZE — G3 / ORYNTH</span>
                <span className="text-volt">RECORDED</span>
              </div>
              <div className="mt-3 space-y-2 font-mono text-[10px] sm:text-[10.5px]">
                <div className="flex flex-col gap-1 sm:flex-row sm:justify-between sm:gap-6">
                  <span className="shrink-0 text-mute">SCOPE</span>
                  <span className="break-all text-mist">
                    cipher/contracts/src/**/*.sol
                  </span>
                </div>
                <div className="flex flex-col gap-1 sm:flex-row sm:justify-between sm:gap-6">
                  <span className="shrink-0 text-mute">HASH</span>
                  <span className="break-all text-mist">{FREEZE}</span>
                </div>
                <div className="flex flex-col gap-1 sm:flex-row sm:justify-between sm:gap-6">
                  <span className="shrink-0 text-mute">TGE VENUE</span>
                  <span className="text-mist">ORYNTH · orynth.dev</span>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2 border-t border-amber-600/25 pt-4 sm:gap-3">
                <a
                  href="#/docs/audit"
                  className="border border-edge2 px-3 py-2.5 font-mono text-[9px] tracking-[0.18em] text-mist transition-colors hover:border-volt/70 hover:text-volt sm:px-4"
                >
                  AUDIT PACK →
                </a>
                <a
          ...
          <aside className="min-w-0 lg:sticky lg:top-24 lg:self-start">
            <div className="border border-volt/50 bg-volt/[0.04]">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-volt/30 px-4 py-3.5 sm:px-5">
                <span className="flex items-center gap-2 font-mono text-[9px] tracking-[0.24em] text-volt">
                  <Server size={11} /> VERIFIER WAITLIST
                </span>
                <span className="font-mono text-[8px] tracking-[0.2em] text-mute">
              ...
              {phase !== "done" ? (
                <div className="space-y-4 p-4 sm:p-5">
                  <p className="text-[12px] leading-[1.7] text-mute">
                    Names now, bonds at the B2 deploy. Waitlist order is signed
                    and public; bonds post in order, 25,000 CENT floor.
                  </p>
                  <div>
                    <label className="mb-1.5 block font-mono text-[8.5px] tracking-[0.22em] text-mute">
                      NODE HANDLE
                    </label>
                    <input
                      value={handle}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {INFRA.map((i) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setInfra(i)}
                          className={`min-h-8 border px-2 py-1.5 font-mono text-[7.5px] tracking-[0.14em] transition-colors ${
                            infra === i
                  ...
                  {joinError && (
                    <p className="text-center font-mono text-[8.5px] tracking-[0.12em] text-red-400">
                      {joinError}
                    </p>
                  )}
                  <p className="text-center font-mono text-[7px] tracking-[0.16em] text-mute/50">
                    SIGNS WITH YOUR DEVICE KEY
                    {key ? ` · ${key.fp}` : ""} · ORDER ROUTES TO OPS QUEUE
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center p-5 text-center sm:p-6">
                  ...
                  <div className="mt-4 font-display text-[18px] font-semibold text-mist">
                    In the queue.
                  </div>
                  <div className="mt-1.5 font-mono text-[9px] tracking-[0.18em] text-mute">
                    POSITION #{queueNumber ?? "—"} · BOND{" "}
                    {bond.toLocaleString()} CENT
                  </div>
                  {sig && (
                  ...
                  <p className="mt-4 font-mono text-[7.5px] leading-[1.8] tracking-[0.16em] text-mute/60">
                    BONDS OPEN ON ENG-B DEPLOY. YOUR ORDER, YOUR HASH, YOUR
                    TURN.
                  </p>
                </div>
              )}
            ...
            <div className="mt-4 border border-edge p-4 sm:p-5">
              <div className="font-mono text-[8.5px] tracking-[0.24em] text-mute">
                WHY THIS ORDER
              </div>
              <p className="mt-2.5 font-mono text-[9px] leading-[1.9] text-mute">
                THE WAITLIST STARTS G1&apos;S CLOCK. THE COUNTER STARTS G4&apos;S.
                BOTH ARE UNRESTARTABLE AND BOTH ARE FREE TO START TODAY.
              </p>
            </div>
          </aside>
        </div>
      ...
      {/* links */}
      <div className="border-t border-edge px-4 py-10 sm:px-6 md:px-12 md:py-14">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {LINKS.map((l) => (
              <a
                ...
                className="group flex min-h-11 items-center justify-between gap-3 border border-edge2 px-4 py-3.5 font-mono text-[11px] tracking-[0.12em] text-mist transition-colors hover:border-volt/60 hover:text-volt"
              >
                {l.label}
                <ArrowUpRight
                  size={13}
