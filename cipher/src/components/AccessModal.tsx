          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-edge px-5 py-3.5">
              <span className="flex items-center gap-2.5">
                ...
                <span className="font-mono text-[9px] tracking-[0.24em] text-mute">CENT.ACCESS.REQUEST</span>
              </span>
            ...
            {phase !== "done" ? (
              <div className="p-5">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div>
                    <label className="mb-1.5 block font-mono text-[8.5px] tracking-[0.24em] text-mute">ROLE</label>
                    <div className="flex flex-wrap gap-1.5">
                      {ROLES.map((r) => (
                        <button key={r} type="button" onClick={() => setRole(r)} className={`border px-2.5 py-2 font-mono text-[8.5px] tracking-[0.16em] transition-colors ${role === r ? "border-volt/70 bg-volt/10 text-volt" : "border-edge2 text-mute hover:text-mist"}`}>
                          {r}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="mb-1.5 block font-mono text-[8.5px] tracking-[0.24em] text-mute">PREFERRED RAIL</label>
                    <div className="flex flex-wrap gap-1.5">
                      {RAILS.map((r) => (
              ...
              className={`flex w-full items-center justify-center gap-2.5 py-4 font-mono text-[11px] font-semibold tracking-[0.22em] transition-all ${
                valid ? "bg-volt text-ink hover:bg-volthot" : "cursor-not-allowed border border-edge2 text-mute/50"
              }`}
                  >
                    {phase === "signing" ? (
                      <>
              ...
              <div className="flex flex-col items-center p-8 text-center">
                ...
                <div className="mt-6 flex gap-3">
                  <button onClick={onClose} className="border border-edge2 px-5 py-3 font-mono text-[10px] tracking-[0.2em] text-mist transition-colors hover:border-volt/70 hover:text-volt">
                    CLOSE
                  </button>
                  <a href="#/docs/specification" onClick={onClose} className="bg-volt px-5 py-3 font-mono text-[10px] font-semibold tracking-[0.2em] text-void transition-colors hover:bg-mist">
                    READ THE SPEC →
                  </a>
                </div>
              </div>
