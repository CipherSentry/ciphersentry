...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="relative scroll-mt-[68px] border-b border-edge bg-void/40">
      <SectionHead
        index="02"
        kicker="HOW IT WORKS"
        title={
      ...
      <div className="grid gap-px border-t border-edge bg-edge lg:grid-cols-3">
        {PILLARS.map((p, i) => (
          <Reveal key={p.code} delay={i * 0.1} className="h-full">
            <div className="group relative flex h-full flex-col overflow-hidden bg-void/90 p-8 transition-colors duration-500 hover:bg-panel md:p-10">
              {/* corner density — one pillar only so the grid stays calm */}
              {i === 1 && (
                <div className="pointer-events-none absolute -right-6 -top-8 h-40 w-48 opacity-50 transition-opacity duration-500 group-hover:opacity-70">
              ...
              <h3 className="relative mt-8 font-display text-[22px] font-semibold tracking-[-0.02em] text-mist">
                {p.title}
              </h3>
              <p className="relative mt-3 text-[13px] leading-[1.75] text-mute">{p.body}</p>
