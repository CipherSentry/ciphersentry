...
/**
 * AsciiFigure — density field from the sentry reference still.
 * `static` = paint once (hero). Live mode throttles hard + larger cells.
 */

const RAMP =
  " .'`^\",:;Il!i><~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$";

// Coarser grid = fewer fillText calls (main cost)
const CELL_W = 8;
const CELL_H = 13;
const LIVE_MS = 90; // ~11fps — smooth enough, cheap on main thread
...
export default function AsciiFigure({
  className = "",
  static: isStatic = false,
}: {
  className?: string;
  /** One-shot paint — use in hero so only the showcase loops. */
  static?: boolean;
}) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Cap DPR — 2x pixel fill is pure cost on this loop
    ...
    const ink = (d: number) => {
      if (d > 0.55) return `rgba(11, 68, 32, ${Math.min(0.72, 0.28 + d * 0.55)})`;
      if (d > 0.28) return `rgba(11, 68, 32, ${Math.min(0.5, 0.18 + d * 0.45)})`;
    ...
    const draw = (timeMs: number) => {
      ...
      // slower, subtler drift
      const ox = reduced ? 0 : Math.sin(t * 0.12) * 0.7;
      const oy = reduced ? 0 : Math.cos(t * 0.1) * 0.5;
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          if (!reduced) {
            ...
            d = Math.max(
              0,
              Math.min(1, d + Math.sin(t * 0.7 + s * 28 + x * 0.08 + y * 0.06) * 0.022),
            );
          }
    ...
    // Debounced resize — avoid thrash mid-scroll/layout
