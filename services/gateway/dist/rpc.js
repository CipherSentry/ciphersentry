...
export function makeDispatcher(ctx) {
    ...
    return async function dispatch(env) {
        const p = (env.params ?? {});
        switch (env.method) {
            case "registry.query": {
                ...
                const out = REGISTRY.filter((a) => a.trust >= minTrust &&
            ...
            case "task.commit": {
                const params = p;
                if (!params.spec || !params.worker)
                    return err(M.SCHEMA, "commit requires spec and worker");
                if (parseFloat(String(params.escrow?.amount ?? "0")) <= 0)
                    return err(M.SCHEMA, "escrow.amount must be > 0");
                const t = addTask({
                    agent: params.worker,
            ...
            case "task.report": {
                const { task_id, hash } = p;
                if (!task_id || !hash)
                    return err(M.SCHEMA, "report requires task_id and hash");
                const t = sim.settleTask(task_id);
                if (!t)
                    return err(M.TIMEOUT, `task ${task_id} unknown outside sim window`);
                t.state = "VERIFYING";
                t.hash = String(hash);
                ctx.emitTask({ ...t });
            ...
            case "verify": {
                const { task_id, quorum = 3 } = p;
                if (!task_id)
                    return err(M.SCHEMA, "verify requires task_id");
                const t = sim.settleTask(task_id);
                if (!t)
                    return err(M.TIMEOUT, `task ${task_id} unknown outside sim window`);
                if (t.state !== "VERIFYING")
                    return err(M.QUORUM_SLOW, `task is ${t.state}, wait for report`);
            ...
            case "task.settle": {
                const { task_id } = p;
                if (!task_id)
                    return err(M.SCHEMA, "settle requires task_id");
                const t = sim.settleTask(task_id);
                if (!t)
                    return err(M.TIMEOUT, `task ${task_id} unknown`);
                if (t.state !== "SETTLED")
                    return err(M.QUORUM_SLOW, `cannot settle while state is ${t.state}`);
                return ok({ task_id, state: t.state });
            }
            case "dispute.open": {
                const { task_id, evidence } = p;
                if (!task_id)
                    return err(M.SCHEMA, "dispute.open requires task_id");
                const t = sim.settleTask(task_id);
                if (!t)
                    return err(M.TIMEOUT, `task ${task_id} unknown`);
                t.state = "DISPUTED";
            ...
            case "operator.rule": {
                const { task_id, ruling, sig } = p;
                if (!task_id || !ruling || !sig)
                    return err(M.SCHEMA, "operator.rule requires task_id, ruling, sig");
                const t = sim.settleTask(task_id);
                if (!t)
                    return err(M.TIMEOUT, `task ${task_id} unknown`);
            ...
            case "stake": {
                const { amount, tier = "T2" } = p;
                const n = parseFloat(String(amount ?? "0"));
                if (!Number.isFinite(n) || n < 25_000)
                    return err(M.CAP_BREACH, "minimum verifier bond is 25,000 CENT");
                return ok({ epoch: ctx.epoch, bond: amount, tier });
            }
            case "events.subscribe": {
                return ok({ subscribed: p.topics ?? [] });
            }
            default:
