...
const S = (s) => keccak256(new TextEncoder().encode(s));
/**
...
export function decodeLog(log, lastState, isEscrow) {
    ...
    if (isEscrow) {
        ...
        if (!rawId)
            return null;
        if (lastState && NEXT[lastState] && !NEXT[lastState].includes(state))
            return null; // skip impossible orders
...
const latestBlock = async (cfg) => BigInt((await rpc(cfg, "eth_blockNumber", [])));
async function getLogs(cfg, address, fromBlock, toBlock) {
    try {
        return await rpc(cfg, "eth_getLogs", [
...
export class ChainWatcher {
    ...
    async poll() {
        try {
            ...
            if (upTo <= this.lastBlock)
                return;
            const from = this.lastBlock + 1n;
            const to = from + BLOCK_STEP > upTo ? upTo : from + BLOCK_STEP;
            const logs = [];
            if (this.cfg.escrowAddress)
                logs.push(...(await getLogs(this.cfg, this.cfg.escrowAddress, from, to)));
            if (this.cfg.batcherAddress)
                logs.push(...(await getLogs(this.cfg, this.cfg.batcherAddress, from, to)));
...
export function makeChainConfigFromEnv() {
    return {
