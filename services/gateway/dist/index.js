...
import { SimDriver } from "./sim.js";
import { makeDispatcher } from "./rpc.js";
import { SubscriptionHub } from "./ws.js";
import { ChainWatcher, makeChainConfigFromEnv } from "./chain.js";
...
async function boot() {
    ...
    fastify.post("/rpc", async (req, reply) => {
        ...
        if (!out.ok)
            return { jsonrpc: "2.0", id: env.id, error: out.error };
...
async function registerChainBinding(hub, sim) {
...
function hubBroadcast(hub, topic, data) {
    hub.broadcast(topic, {
        jsonrpc: "2.0",
...
const isEntry = process.argv[1]?.endsWith("index.js");
if (isEntry) {
    void boot().catch((e) => {
        console.error(e);
        process.exit(1);
