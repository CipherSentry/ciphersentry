...
export const outputHashOf = (s) => `0x${hex(fnv32(s))}${hex(fnv32(s, 0x9af2be))}`;
...
export function injectedNowMs(taskId) {
...
export class DeterministicSandbox {
    async run(req) {
        ...
        let mod;
        try {
            mod = await WebAssembly.compile(req.wasm);
        }
        catch (e) {
        ...
        for (const imp of WebAssembly.Module.imports(mod)) {
            if (imp.module !== "env")
                return { ok: false, error: `import namespace "${imp.module}" forbidden`, ms: Date.now() - started, logs };
            if (!FROZEN_IMPORTS.includes(imp.name)) {
        ...
        const env = {
            ...
            cent_budget_checkpoint: (remaining) => {
                if (remaining <= 0)
                    budgetOk = false;
                return budgetOk ? 1 : 0;
            },
        ...
        const runtimeMemory = instance.exports.memory ?? memory;
        ...
        ensureMemory(runtimeMemory, INPUT_PTR + inputBytes.length);
        new Uint8Array(runtimeMemory.buffer).set(inputBytes, INPUT_PTR);
        const execute = instance.exports.cent_execute;
        ...
        console.log('[debug] packed=', packed, 'typeof=', typeof packed);
        if (!budgetOk) {
        ...
        const outPtr = typeof packed === 'bigint' ? Number(packed & 0xffffffffn) : packed >>> 0;
        console.log('[debug] outPtr=', outPtr, 'byteLength=', runtimeMemory.buffer.byteLength);
        const len = readU32(runtimeMemory, outPtr);
        console.log('[debug] len=', len);
        const prefix = decodeMemory(runtimeMemory, outPtr, len);
        let outputJson;
        try {
...
/* ------------------------------ helpers ----------------------------------- */
export function canonicalHashForTask(taskId) {
    return outputHashOf(canonicalize({ task: taskId, op: "seed" })).slice(2);
}
function ensureMemory(memory, needBytes) {
...
function decodeMemory(memory, ptr, len) {
